...
class UpdateService {
  ...
  constructor(academicYear, semester) {
    ...
    this.startDate = this.getSemesterStartDate(
      this.academicYear,
      this.semester
    );
  }
  ...
  async getModuleDetails(moduleCode) {
    try {
      ...
      const semesterData = data.semesterData.find(
        (s) => s.semester === this.semester
      );
  ...
  async deleteStudentModuleMapping() {
    console.log(
      "Deleting all student module mapping in user_module_mapping table..."
    );
    try {
      await db.none(
        "TRUNCATE TABLE request_management.user_module_mapping CASCADE"
      );
    } catch (error) {
  ...
  async deleteModulesAndClasses() {
    ...
    try {
      await db.none("TRUNCATE TABLE request_management.modules CASCADE");
    ...
    try {
      ...
      if (lessonRows.length > 0) {
        ...
        try {
          ...
          console.error(
            "Bulk insert for request_management.classes failed:",
            err
          );
        }
      }
      ...
      console.error(`populateModAndClass error ${error}`);
    }
  }
