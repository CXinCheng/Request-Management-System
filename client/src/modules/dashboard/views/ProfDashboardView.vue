  ...
  <v-container>
    <v-row>
      <v-col cols="12" sm="12" md="12" lg="4" xl="4">
        <v-card>
          <v-card-text>
            <VueUiDonut :dataset="dataset" :config="config"></VueUiDonut>
          </v-card-text>
        </v-card>
      </v-col>
...
const viewModule = (moduleCode) => {
  router.push({
    ...
    params: {
      ...
      moduleName: modules.value.find((module) => module.code === moduleCode)
        .name,
    },
  });
};
...
onMounted(async () => {
  try {
    ...
    let response = await gatewayApiService.getModulesWithRequestsByProfessor(
      profId
    );
    if (response.success) {
    ...
    if (response.success) {
      const requestsByModule = response.data.reduce((acc, request) => {
        ...
        if (request.status === "Pending") acc[request.module_code].pending++;
