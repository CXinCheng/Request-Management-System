<template>
    <div v-if="request">
        ...
        <p>
            ...
            {{ formatDate(request.modified_at) }}
        </p>
        <p>
            <strong>Dates of request:</strong>
            {{ formatDate(request.start_date_of_leave) }} -
            {{ formatDate(request.end_date_of_leave) }}
        </p>
    ...
    methods: {
        async fetchRequest() {
            try {
                ...
                const response =
                    await requestApiService.getRequestDetailsByStudent(
                        this.studentId,
                        this.requestId
                    );
